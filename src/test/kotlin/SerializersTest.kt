...
class SerializersTest {
    ...
    fun `test float32 serializer`() {
        val values = listOf(0.0f, 1.0f, -1.0f, 3.14f, Float.MAX_VALUE, Float.MIN_VALUE, Float.NaN, Float.POSITIVE_INFINITY, Float.NEGATIVE_INFINITY)
    ...
    fun `test float64 serializer`() {
        val values = listOf(0.0, 1.0, -1.0, 3.14159265359, Double.MAX_VALUE, Double.MIN_VALUE, Double.NaN, Double.POSITIVE_INFINITY, Double.NEGATIVE_INFINITY)
    ...
    fun `test string serializer`() {
        val values = listOf("", "hello", "world", "Hello, 世界!", "🚀", "\n\t\r", "A very long string that exceeds normal buffer sizes and should test the streaming capabilities of the serializer properly")
    ...
    fun `test bytes serializer - dense flavor`() {
        ...
        for (value in values) {
            ...
            assertTrue(denseJson.startsWith("\"") && denseJson.endsWith("\""), 
    ...
    fun `test timestamp serializer - dense flavor`() {
        val timestamps = listOf(
            ...
            Instant.ofEpochMilli(System.currentTimeMillis()) // Use epoch millis to avoid nanosecond precision issues
        )
